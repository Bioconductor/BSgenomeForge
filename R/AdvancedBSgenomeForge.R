### =========================================================================
### Advanced BSgenomeForge functions
### -------------------------------------------------------------------------
                                            ...
                                            ondisk_seq_format, verbose=TRUE)
{
    ...
    if (ondisk_seq_format == "fa") {
        ...
        Rsamtools::indexFa(dest_filepath)
        if (verbose)
            cat("DONE\n")
    } else {
        ## "fa.rz" format
        ...
        Rsamtools::razip(dest_filepath, dest=farz_filepath)
        ...
        if (verbose)
            cat("Indexing compressed FASTA file '", farz_filepath,
                "' ... ", sep="")
        Rsamtools::indexFa(farz_filepath)
        if (verbose)
            cat("DONE\n")
    }
}
                            ...
                            verbose=TRUE)
{
    ...
    if (ondisk_seq_format == "rda") {  # "rda" format
        ...
        fafile <- Rsamtools::FaFile(fa_filepath)
        ...
        seq <- Rsamtools::scanFa(fafile, param=param)[[1L]]
    }
...
setMethod("forgeBSgenomeDataPkg", "BSgenomeDataPkgSeed",
    function(x, seqs_srcdir=".", destdir=".", replace=FALSE, verbose=TRUE)
    {
        ...
        symvals <- list(
            ...
            PKGAUTHOR=x@Author,
            PKGMAINTAINER=x@Maintainer,
            BSGENOMEVERSION=BSgenome_version,
            SUGGESTS=x@Suggests,
            PKGLICENSE=x@License,
            ORGANISM=x@organism,
...
.readSeedFile <- function(file, verbose=TRUE)
{
    ...
    if (file.exists(file)) {
        ...
        file0 <- file
        seed_dir <- system.file("extdata", "seeds", package="BSgenomeForge")
...
setMethod("forgeMaskedBSgenomeDataPkg", "MaskedBSgenomeDataPkgSeed",
    function(x, masks_srcdir=".", destdir=".", verbose=TRUE)
    {
        ...
        template_path <- system.file("pkgtemplates", "MaskedBSgenome_datapkg",
                                     package="BSgenomeForge")
        BSgenome_version <- installed.packages()['BSgenome','Version']
        symvals <- list(
            PKGTITLE=x@Title,
        ...
        seqs_destdir <- dirname(BSgenome:::seqlengthsFilepath(ref_bsgenome@single_sequences))
        pkgdir <- file.path(destdir, x@Package)
